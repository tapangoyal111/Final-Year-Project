...
ETX=50*0.000000001;
%Energy required to recieve the data
ERX=50*0.000000001;
%some other factors
Efs=10e-12;
...
Emp=0.0013e-12;
EDA=5*0.000000001;
%Maximum Rounds if any node alive
rmax=1000;
%threshhold distance
do=sqrt(Efs/Emp);
Et=0;
...
for h=1:1
    ...
    S(n+1).yd=sink.y;
    Et=0;
% S(n+1) is the array of n nodes/sensors + base station 
    %with following atttributes
    % xd=x coordinate
    % yd=y coordinate
    % distance=distance from base station **2
    % type=Node or cluster head
    % E=Energy remaining 
    % G=
    % xd=x coordinate
